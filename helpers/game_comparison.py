...
def score_candidate(t_fields, j_game):
    ...
    if t_time == "Not scheduled" and (j_time == "07:00" or j_time == "00:00"):
        ...
        t_game_time and (t_game_time - timedelta(hours=1)).time() == j_game_time.time()
    ):  # Arrival time
        score += 30
        ...
        reason += (
            f"Ottelun oikea alkamisaika on klo: {t_time}, mutta Jopoxissa se on klo {j_time}. "
        )
        color_score_temp += 1
    ...
    if location_match_score > 80:
        ...
        if t_location_number and j_location_number and t_location_number[0] != j_location_number[0]:
            ...
            color_score_temp += 1
            reason += (
                f"Ottelu pelataan paikassa: {location}, mutta Jopoxiin on merkattu: {j_location}. "
            )
    ...
    if "Lisätiedot" in j_game and j_game["Lisätiedot"]:
        if small_area_game:
            if (
                "pienpeli" not in j_game["Lisätiedot"].lower()
                and "pienpeli" not in j_game["joukkueet"].lower()
            ):
                reason += "Kyseessä on pienpeli, mutta siitä ei ole mainintaa Jopoxissa. "
                color_score_temp += 1
            elif (
                "pienpeli" in j_game["Lisätiedot"].lower()
                or "pienpeli" in j_game["joukkueet"].lower()
            ):
                score += 20
...
def compare_games(jopox_games, tulospalvelu_games, jopox_links=None):
    ...
    managed_games = [
        managed_game for managed_game in tulospalvelu_games if managed_game["Type"] != "follow"
    ]
    ...
    for entry in entries:
        ...
        logger.debug(
            ...
            entry["t_game"].get("Game ID"),
            linked_match.get("uid"),
            scored_link[2],
        )
