    ...
    try:
        for team in selected_teams:
            ...
            user_team = UserTeam.query.filter_by(
                user_id=current_user.id, team_id=team_id, relationship_type=relationship_type
            ).first()
            if user_team:
        ...
        managed_teams = [
            ...
            if team.team_user_entries[0].relationship_type == "manage"
        ]
        followed_teams = [
            {"team_name": team.team_name, "stat_group": team.stat_group, "id": team.id}
            for team in current_user.teams
            if team.team_user_entries[0].relationship_type == "follow"
        ]
